...
class XGBoostSkill(BaseSkill):
    name = "xgboost"
    description = "XGBoost regressor with walk-forward validation for price forecasting."
    ...
    def execute(self, params: dict[str, Any]) -> SkillResult:
        ...
        # Train score on last fold's full train set
        last_fold = folds[-1]
        X_all_train = last_fold.train[feature_cols].values
        y_all_train = last_fold.train[target_col].values
        train_preds = last_model.predict(X_all_train)
        train_score = metric_registry.evaluate(metric_name, y_all_train, train_preds)

        log.info("xgboost_trained", val_score=round(val_score, 4), folds=len(folds))
